<?php
if ( ! defined( 'NEWS_THEME_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'NEWS_THEME_VERSION', '1.0.0' );
}

if ( ! function_exists( 'news_theme_setup' ) ) :
	/**
	...
	function news_theme_setup() {
		 ...
		 * If you're building a theme based on news_theme, use a find and replace
		 * to change 'news-theme' to the name of your theme in all the template files.
		 */
...
if ( ! defined( 'IMSANITY_VERSION' ) ) {
	...
	define( 'NEWS_THEME_IMSANITY_DEFAULT_MAX_WIDTH', 2000 );
	define( 'NEWS_THEME_IMSANITY_DEFAULT_MAX_HEIGHT', 2000 );
	define( 'NEWS_THEME_IMSANITY_DEFAULT_QUALITY', 90 );
 ...
 */
function news_theme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'news_theme_content_width', 1440 );
}
add_action( 'after_setup_theme', 'news_theme_content_width', 0 );
...
function sp_the_header_logo() {
	...
	<a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home" aria-label="<?php echo get_bloginfo( 'name' ); ?>">
		<?php if ( ! get_theme_mod( 'custom_logo' ) ) : ?>
			<img width="191" height="61" class="site-header__logo" src="<?php echo sp_get_asset( 'logo.svg' ); ?>" alt="">
		<?php else : ?>
